...
export default function App() {
  ...
  const handleConfirmOrder = (customMeal?: CustomMeal) => {
    ...
    const newOrder: Order = {
      ...
      items: [...itemsToOrder],
      total: itemsToOrder.reduce((sum: number, item: CartItem) => sum + (item.data.price * item.quantity), 0) + 5,
      status: 'processing',
          ...
          {currentView === 'home' && (
            ...
            >
              <HomePage 
                ...
                onExploreClick={() => handleTabChange('menu')}
              />
            </motion.div>
          )}
            ...
            >
              <CartPage 
                ...
                onUpdateQuantity={(id, delta) => {
                  setCart(cart.map((item: CartItem) => item.id === id ? { ...item, quantity: Math.max(1, item.quantity + delta) } : item));
                }}
                onRemove={(id) => setCart(cart.filter((item: CartItem) => item.id !== id))}
